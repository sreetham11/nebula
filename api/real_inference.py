...
# Same load-bearing import order as api/main.py, repeated here so this
# module is also safe to import on its own: xgboost before torch (macOS
# OpenMP double-load segfaults xgboost's first predict), torch before
# numpy/pandas (Windows libiomp5md.dll clash breaks torch's c10.dll).
import xgboost  # noqa: E402 -- must precede torch
import torch  # noqa: E402 -- must precede numpy/pandas
...
def load_real_models(models_dir):
    ...
    rail_clf = load_rail_classifier(p("real_rail_classifier.pkl"))
    rail_cols = []
...
# ---------------------------------------------------------------------------
# Rail classifier loading
# ---------------------------------------------------------------------------

def _recover_rail_classifier(path):
    """
    Rebuilds the XGBClassifier from real_rail_classifier.pkl when a plain
    unpickle fails.

    The pickle stores the booster through XGBoosterSerializeToBuffer, whose
    output is a {Config, Model} UBJSON container. That buffer is only
    readable by the exact xgboost build that wrote it -- XGBoost documents
    pickles as non-portable and recommends save_model/load_model instead --
    so on the pinned 3.4.1 wheel the deserialize raises "input stream
    corrupted" even though the file is byte-for-byte what was committed.

    The Model half of the container IS the portable save_model payload, so
    we unpickle with the booster stubbed out, split the container on its
    "Model" key, and load that half through Booster.load_model(). The
    resulting classifier is identical -- same 300 rounds, same 387 features
    in the same order, read out of the model itself rather than reconstructed.
    """
    captured = {}

    class _BoosterStub(xgboost.Booster):
        def __init__(self, *args, **kwargs):
            pass

        def __setstate__(self, state):
            captured["buffer"] = bytes(state["handle"])
    ...
    with open(path, "rb") as fh:
        clf = _Unpickler(fh).load()

    buf = captured.get("buffer")
    if buf is None:
        raise RuntimeError("no serialized booster found in real_rail_classifier.pkl")

    # UBJSON object key: 'L' + int64 length + the key's characters.
    key = b"L" + struct.pack(">q", len(b"Model")) + b"Model"
    start = buf.find(key)
    if start < 0:
        raise RuntimeError("serialized booster is not a {Config, Model} container")
    model_bytes = buf[start + len(key):-1]  # drop the container's closing '}'

    booster = xgboost.Booster()
    booster.load_model(bytearray(model_bytes))

    clf._Booster = booster
    if getattr(clf, "feature_names_in_", None) is None:
        if not booster.feature_names:
            raise RuntimeError("recovered booster carries no feature names")
        clf.feature_names_in_ = np.array(booster.feature_names, dtype=object)
    return clf


def load_rail_classifier(path):
    try:
        return joblib.load(path)
    except xgboost.core.XGBoostError:
        return _recover_rail_classifier(path)
